...
class Dataset_RGBProMP(data.DatasetOrdinary, data.DatasetRGB):
        ...
        def prepare_data(self, val_frac=0.0, N_test=0, random_state=None,use_val_in_train=False):
            ...
            n_test = len(idx_test)
            n_val = len(idx_val)
            X_train, y_train = {"encoded":self.samples["img_enc"][idx_train].astype('float64'),"X": self.samples["X"][idx_train].astype('float32'),"Y": self.samples["Y"][idx_train].astype('float32')}, \
            ...
            X_val, y_val = {"encoded":self.samples["img_enc"][idx_val].astype('float64'),"X": self.samples["X"][idx_val].astype('float32'),"Y": self.samples["Y"][idx_val].astype('float32')}, \
            ...
            X_test, y_test = {"encoded":self.samples["img_enc"][idx_test].astype('float64'),"X": self.samples["X"][idx_test].astype('float32'),"Y": self.samples["Y"][idx_test].astype('float32')}, \
