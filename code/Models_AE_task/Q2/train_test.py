...
from output import plot_loss,My_metric,My_metric_final_point
from ProMP import ProMP
import json
import config_Q2 as cfg
...
class Experiment_ProMPs(Experiment):
    def __init__(self):
        ...
        encoder_decoder_model = tf.keras.models.load_model(self.cfg.ENCODER_MODEL_PATH)
        # Extract the bottleneck layer
        bottleneck_layer = encoder_decoder_model.get_layer('bottleneck').output
        # Build the complete encoder model.
        encoder = Model(encoder_decoder_model.input, bottleneck_layer)
    ...
    def eval(self, load_model_name):
        ...
        for i in range(n_test):
            ...
            # final_mean_pred = MEAN_TRAJ_pred[[-1][0]]
            # final_mean = MEAN_TRAJ_true[[-1][0]]
            # final_right = final_mean + 3*STD_TRAJ_true[[-1][0]]
            # final_right_pred = final_mean_pred + 3*STD_TRAJ_pred[[-1][0]]
            #
            # metric += My_metric_final_point(final_mean, final_mean_pred, final_right, final_right_pred)
...
if __name__ == "__main__":
    ...
    TEST THE MODEL
    '''
    Experiment_ProMPs.eval(load_model_name='model_09_02__18_08')
