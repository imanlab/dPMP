...
class Experiment_ProMPs(Experiment):
    def __init__(self):
        ...
        vae = tf.saved_model.load(self.cfg.ENCODER_MODEL_PATH)
        encoder = vae.encoder
    ...
    def train(self):
        ...
        print('encoded train:   ', np.shape(encoded_train),encoded_train.dtype)
        print('mean train:   ', mean_train.shape)
        print('L train:   ', L_train.shape)
        print('yt train:   ', yt.shape,yt.dtype)
        print('encoded val:   ', np.shape(encoded_val),encoded_val.dtype)
        print('mean val:   ', mean_val.shape)
        print('L val:   ', L_val.shape)
        print('yt val:   ', yv.shape,yv.dtype)
        ...
        '''Train '''
        history = self.model.fit([encoded_train, x_train, y_train], yt, epochs=self.cfg.epochs, batch_size=self.cfg.batch,validation_data=([encoded_val, x_val, y_val], yv), callbacks=self.callbacks)
    ...
    def eval(self, load_model_name):
        ...
        x_true = X_true["X"]
        ''' Predict'''
        ypred = model.predict([encoeded_true,x_true,y_true])
...
if __name__ == "__main__":
    ...
    TEST THE MODEL
    '''
    Experiment_ProMPs.eval(load_model_name='model_10_02__16_15')
